...
def train_one_epoch(model, model_without_ddp, data_loader, optimizer, device, epoch, log_writer=None, args=None):
    ...
    use_tqdm = tqdm is not None and misc.is_main_process()
    if use_tqdm:
        data_iter = tqdm(data_iter, total=len(data_loader), desc=f"Train E{epoch}", dynamic_ncols=True)
    ...
    for data_iter_step, batch in enumerate(data_iter):
        ...
        if args.dataset_mode == 'paired':
            y_cond, x, labels = batch
            y_cond = y_cond.to(device, non_blocking=True).to(torch.float32).div_(255)
            y_cond = y_cond * 2.0 - 1.0
        ...
        amp_ctx = torch.amp.autocast('cuda', dtype=torch.bfloat16) if torch.cuda.is_available() else nullcontext()
        with amp_ctx:
            loss = model(x, labels, y_cond=y_cond)
        ...
        if args.max_train_steps > 0 and (data_iter_step + 1) >= args.max_train_steps:
            print(f"Reach max_train_steps={args.max_train_steps}, stop this epoch early for smoke test.")
            break
...
def evaluate(model_without_ddp, args, epoch, batch_size=64, log_writer=None):
    ...
    for i in range(num_steps):
        ...
        labels_gen = torch.tensor(labels_gen, dtype=torch.long, device=device)
    ...
    elif log_writer is not None and torch_fidelity is None:
        print("torch_fidelity is not installed. Skip FID/IS evaluation.")
