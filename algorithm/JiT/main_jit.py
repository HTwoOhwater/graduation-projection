...
def get_args_parser():
    ...
    parser.add_argument('--loss_type', default='l2', choices=['l2', 'l1', 'l1_l2'],
                        help='Velocity loss type. Keeps target as v while changing penalty form.')
    parser.add_argument('--loss_l1_weight', default=0.5, type=float,
                        help='L1 weight when loss_type is l1_l2')
    parser.add_argument('--loss_l2_weight', default=0.5, type=float,
                        help='L2 weight when loss_type is l1_l2')
    ...
    parser.add_argument('--dataset_mode', default='paired', choices=['paired', 'imagenet'],
                        help='Dataset mode: paired restoration or ImageNet class generation')
    parser.add_argument('--train_split', default='train', type=str,
                        help='Dataset split to use for training')
    parser.add_argument('--lq_dirname', default='haze_images', type=str,
                        help='Sub-directory name of low-quality/condition images under split')
    parser.add_argument('--gt_dirname', default='original_images', type=str,
                        help='Sub-directory name of ground-truth images under split')
    parser.add_argument('--pairing_mode', default='stem', choices=['stem', 'meta'],
                        help='Pairing strategy for paired dataset')
    parser.add_argument('--pair_meta', default='', type=str,
                        help='Optional meta file for explicit LQ/GT pairing')
    parser.add_argument('--image_channels', default=3, type=int,
                        help='Number of channels for target image')
    parser.add_argument('--use_paired_label_condition', action='store_true',
                        help='Use optional labels from paired dataset as class condition')
    parser.add_argument('--max_train_steps', default=0, type=int,
                        help='Max training steps per epoch for quick smoke test (0 means full epoch)')
...
def main(args):
    ...
    if args.dataset_mode == 'paired':
        ...
        if args.online_eval or args.evaluate_gen:
            print('Paired mode does not support generative FID evaluation yet. Disable online_eval/evaluate_gen.')
        ...
        dataset_train = datasets.ImageFolder(os.path.join(args.data_path, args.train_split), transform=transform_train)
    print(dataset_train)
